...
El presente trabajo se enmarca en este contexto, con el desarrollo de un prototipo funcional de máquina lanzapelotas de tenis portátil y autónoma. La figura~\ref{fig:contextoGeneral} ilustra, de manera esquemática, el lugar que ocupa el sistema desarrollado dentro de una sesión de entrenamiento típica, con el jugador, la cancha y la máquina como elementos principales del contexto de uso.

\begin{figure}[htbp]
	\centering
	\includegraphics[scale=.37]{./Figures/questionMark.png}
	\caption{Ubicación de la máquina lanzapelotas dentro de una sesión de entrenamiento de tenis (imagen ilustrativa a reemplazar).}
...
\begin{table}[htbp]
	\centering
	\caption{Comparación de soluciones existentes para el lanzamiento asistido de pelotas de tenis (tabla a completar con los valores relevados para cada solución concreta).}
	\label{tab:comparativaEstadoArte}
	\begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
		\hline
		\tabhead{Solución} & \tabhead{Controlador} & \tabhead{Memoria} & \tabhead{Conectividad} & \tabhead{Costo aprox.} \\
		\hline
		Máquina comercial gama media & [Completar] & [Completar] & [Completar] & [Completar] \\
		\hline
		Máquina comercial gama alta & [Completar] & [Completar] & [Completar] & [Completar] \\
		\hline
		Prototipo académico / DIY & [Completar] & [Completar] & [Completar] & [Completar] \\
		\hline
		Prototipo propio (este trabajo) & Microcontrolador ESP32-S3 & Memoria flash no volátil interna & Bluetooth (en desarrollo) & Costo de componentes acotado \\
		\hline
	\end{tabularx}
\end{table}
